    ...
    const apiCall = async () => {
      ...
      try {
        const { data } = await axios.get(
          `${API_URL}/tasks/${userId}/activities`,
          {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-6xl bg-white shadow-lg rounded-lg p-6">
        ...
        <Link to={`/ActivityAdd/${userId}`} className="text-blue-50">
          <button
            className="px-4 py-2 text-black rounded-lg bg-green-500"
            disabled={isDisabled}
          >
            Add
          </button>
        ...
        <Link to={`/Assignto/${userId}`} className="text-blue-50">
          <button
            className="px-4 py-2 text-sm text-white ml-2 bg-blue-600 rounded-lg mr-2"
            disabled={isDisabled}
          >
            Assign to
          </button>
                ...
                userdata.map((item, index) => (
                  <tr key={item.activityid}>
                    ...
                    <td className="py-3 px-6 text-center">
                      <button
                        className="px-4 py-2 text-sm text-white bg-blue-600 rounded-lg mr-2"
                        disabled={isDisabled}
                      >
                        Edit
                      </button>
                      <button
                        className="px-4 py-2 text-sm ml-1 text-white bg-red-600 rounded-lg"
                        disabled={isDisabled}
                      >
                        Delete
                      </button>
...
//     }
//     try {
//       const response = await axios.get(`${API_URL}/tasks/${id}/activities`, {
//         headers: { Authorization: `Bearer ${token}` },
//       });
...
//           <button
//             className={`px-4 py-2 text-black rounded-lg bg-green-500 ${
//               isDisabled ? " cursor-not-allowed" : "bg-green-500"
//             }`}
//             disabled={isDisabled}
//           >
//             Add
...
//           <button
//             className={`px-4 py-2 text-sm text-white ml-2 bg-blue-600  rounded-lg mr-2 ${
//               isDisabled ? " cursor-not-allowed" : "to-blue-600"
//             }`}
//             disabled={isDisabled}
//           >
//             Assign to
...
//           type="button"
//           onClick={handleDoneClick}
//           className={`px-4 py-2 text-sm text-white ml-0 bg-green-600  rounded-lg mr-2 ${
//             isDisabled ? " cursor-not-allowed" : "bg-green-600"
//           }`}
//           disabled={isDisabled}
//         >
//           Done
...
//                     <td className="py-3 px-6 text-center">
//                       <button
//                         className={`px-4 py-2 text-sm text-white bg-blue-600 hover:bg-blue-600 rounded-lg mr-2${
//                           isDisabled ? " cursor-not-allowed" : "to-blue-600"
//                         }`}
//                         disabled={isDisabled}
//                       >
...
//                       </button>
//                       <button
//                         className={`px-4 py-2 text-sm ml-1 text-white bg-red-600 hover:bg-red-600 rounded-lg ${
//                           isDisabled
